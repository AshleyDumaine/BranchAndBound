...
int dp_knapsack(int idx, int cap, std::vector<std::vector<int> > contents, std::vector<std::vector<int> > input_list) {
    if (cap == 0 || idx == input_list.size() - 1) {
        return 0;
    }
    else if (cap - input_list.at(idx).at(2) < 0) {
        return dp_knapsack(idx+1, cap, contents, input_list);
    }
    else {
	contents.push_back(input_list.at(idx));
        return std::max(dp_knapsack(idx+1, cap, contents, input_list), dp_knapsack(idx+1, cap-input_list.at(idx).at(2), contents, input_list) + input_list.at(idx).at(1));
    }
}
...
int main(int argc, char *argv[]) {
	...
	std::cout << std::fixed;
        std::cout << "optimal value: " << dp_knapsack(0, cap, contents, input_list) << std::endl; 
